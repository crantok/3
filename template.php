...
/*
 * Adds some markup to the footer contact webform.
 */
function pixture_reloaded_preprocess_webform_form(&$vars) {
  
  // Code taken from the webform.module
  if (isset($vars['form']['details']['nid']['#value'])) {
    $nid = $vars['form']['details']['nid']['#value'];
  }
  else {
    $nid = $vars['form']['submission']['#value']->nid;
  }
  ...
  if ($nid == 4) {
    ...
    array_unshift(
      ...
      array( 'stuff' => array(
        '#type' => 'item',
        '#prefix' => '<div class="messageheader">',
        '#suffix' => '</div>',
        '#value' => $node->body,
    ) ) );
    $vars['form']['submitted']['name']['#prefix'] = '<div class="messagefields">' . $vars['form']['submitted']['your_name']['#prefix'];
    $vars['form']['submitted']['message']['#prefix'] = '</div><div class="messagebody">' . $vars['form']['submitted']['enquiry']['#prefix'];
    $vars['form']['submit']['#suffix'] = $vars['form']['submit']['#suffix'] . '</div>';
  }
}
